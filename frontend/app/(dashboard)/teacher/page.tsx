...
export default function TeacherPage() {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === "haptic_feedback") {
          ...
          setTimeout(() => setCurrentPhoneme(null), dur + 300);
        }
      } catch {
        // ignore parse errors
      }
  ...
  const loadLesson = async () => {
    try {
      const r = await fetch(`http://localhost:8000/lessons/load/${selectedLesson}`, { method: "POST" });
      if (r.ok) {
        // Fetch the lesson JSON for timeline
        const lessonR = await fetch(`http://localhost:8000/lessons`);
        const lessonData = await lessonR.json();
        // Also load the actual phonemes for the timeline
        // We'll parse the lesson file directly via a simple fetch
      }
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950 text-white">
      ...
      <main className="flex-1 flex overflow-hidden">
        {/* Left: Video panel (takes ~60% width) */}
        <div className="flex-[3] p-4 flex flex-col gap-4 overflow-y-auto">
          {/* Large video with landmarks overlay */}
        ...
        {/* Right: Controls sidebar (takes ~40% width) */}
        <div className="flex-[2] p-4 border-l border-gray-800 overflow-y-auto space-y-4">
          {/* Lesson Selector */}
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="w-full h-3 bg-gray-800 rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full transition-all duration-75 bg-gradient-to-r from-green-500 to-emerald-400"
                    style={{ width: `${Math.min(mouthOpenness * 100, 100)}%` }}
                  />
                </div>
              </div>

              {/* Lip width (distance between outer corners) */}
              {landmarks.length > 0 && (() => {
                const left = landmarks.find(l => l.index === 78);
                const right = landmarks.find(l => l.index === 308);
                const width = left && right ? Math.abs(right.x - left.x) : 0;
                return (
                  <div>
                    <div className="flex justify-between text-xs text-gray-500 mb-1">
                      <span>Lip Width</span>
                      <span className="font-mono">{(width * 100).toFixed(1)}%</span>
                    </div>
                    <div className="w-full h-3 bg-gray-800 rounded-full overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-75 bg-gradient-to-r from-blue-500 to-indigo-400"
                        style={{ width: `${Math.min(width * 100 * 5, 100)}%` }}
                      />
                    </div>
                  </div>
                );
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
            ...
            <p className="text-xs text-gray-600 mb-3">
              Send a test vibration to all connected phones.
            </p>
            <div className="grid grid-cols-2 gap-2">
              {["vowel", "consonant", "buzz", "silence"].map((type) => (
                <button
                  key={type}
                  onClick={() => {
                    // Send test haptic via the backend
                  ...
                  className={`px-3 py-2 rounded-lg text-xs font-medium capitalize transition-colors ${
                    type === "vowel"
                      ...
                      : type === "buzz"
                      ? "bg-yellow-900/30 border border-yellow-700 text-yellow-400 hover:bg-yellow-900/50"
